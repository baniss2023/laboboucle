...
function wireExport(){
  ...
  btn.addEventListener("click", () => {
    ...
    const missingProofs = getMissingProofs();
    if(missingProofs.length > 0){
      showExportWarningModal(missingProofs.length, launchExport);
      return;
    }
...
function storageKeyForAnswer(id){ return `answer:${id}`; }

function wireAnswerFields(){
  document.querySelectorAll('textarea[id]').forEach(field => {
    try{
      const saved = localStorage.getItem(storageKeyForAnswer(field.id));
      if(saved !== null) field.value = saved;
    }catch(_e){}

    const sync = () => {
      try{ localStorage.setItem(storageKeyForAnswer(field.id), field.value); }catch(_e){}
    };
...
function buildExportWarningMessage(missingCount){
  if(missingCount === 1){
    return 'Il manque encore 1 capture obligatoire. Voulez-vous continuer vers l’exportation PDF ?';
  }
  if(missingCount === 2){
    return 'Il manque encore 2 captures obligatoires. Voulez-vous continuer vers l’exportation PDF ?';
  }
  return 'Certaines captures obligatoires ne sont pas encore insérées. Voulez-vous continuer vers l’exportation PDF ?';
}

function showExportWarningModal(missingCount, onConfirm){
  ...
  modal.setAttribute('aria-labelledby', 'confirmExportTitle');

  modal.innerHTML = `
    <h2 class="confirm-modal-title" id="confirmExportTitle">Vérification avant export</h2>
    <p class="confirm-modal-text">${buildExportWarningMessage(missingCount)}</p>
    <div class="confirm-modal-actions">
      <button type="button" class="actionbtn actionbtn-secondary" data-modal-action="cancel">Retourner au labo</button>
      <button type="button" class="actionbtn" data-modal-action="confirm">Oui</button>
    </div>
  `;
